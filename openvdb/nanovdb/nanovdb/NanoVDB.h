...
public:
    ...
    //***** by yanhan **** 
    // __hostdev__ AccessorType getValAccessor() {return AccessorType(this->tree().root()); }
...
private:
    ...
    //********************* try to get non-const tile by yanhan ***********
//     __hostdev__ Tile* findTile(const CoordType& ijk)
//     {
//         Tile* tiles = reinterpret_cast<Tile*>(this + 1);
//         const auto  key = DataType::CoordToKey(ijk);
// #if 1   // switch between linear and binary seach
//         for (uint32_t i = 0; i < DataType::mTableSize; ++i) {
//             if (tiles[i].key == key) return &tiles[i];
...
//     }
    ...
    //******************************* try to support setValueOnly by yanhan (RootNode) *********************
    //******************************************************************************************************

    // template<typename AccT>
    // __hostdev__ void setValueAndCache(const CoordType& ijk, const AccT& acc, const ValueType& v)
    // {
    //     if (Tile* tile = this->findTile(ijk)) {
    //         if (tile->isChild()) {
    //             auto *child = this->getChild(tile);
    //             acc.insert(ijk, child);
    //             child->setValueAndCache(ijk, acc, v);
    //             return;
    //         }
    //         tile->setValue(ijk, true, v);
    //         return;
    //     }
    // }
...
private:
    ...
    //******************************* try to support setValueOnly by yanhan (InternalNode) *****
    //******************************************************************************************
    // template<typename AccT>
    // __hostdev__ void setValueAndCache(const CoordType& ijk, const AccT& acc, const ValueType& v)
    // {
...
struct NANOVDB_ALIGN(NANOVDB_DATA_ALIGNMENT) LeafData
{
    ...
    // __hostdev__ ValueType* getValueAds
...
private:
    ...
    //******************************* try to support setValueOnly by yanhan (LeafNode) *********************
    //*******************************************************************************************

    // template<typename AccT>
    // __hostdev__ void setValueAndCache(const CoordT& ijk, const AccT&, const ValueType& v) { return this->setValueOnly(ijk, v); }
...
#endif
    mutable const RootT* mRoot;  //**********origin:[mutable const RootT* mRoot;] ********* by yanhan
...
public:
    ...
    /// @brief Constructor from a root node
    __hostdev__ ReadAccessor(const RootT& root)  //*********origin:[ const RootT& root ] ******** by yanhan
#ifdef USE_SINGLE_ACCESSOR_KEY
        : mKey(CoordType::max())
#else
...
// ************************ try to support setValueOnly by yanhan (Accessor) *************************************
//  **************************************************************************************************
// #include <stdio.h>
//     __hostdev__ void setValueOnly(const CoordType& ijk, const ValueType& v)
//     {
// #ifdef USE_SINGLE_ACCESSOR_KEY
//         const CoordValueType dirty = this->computeDirty(ijk);
// #else
...
//             // printf("leaf");
//             ((LeafT*)mNode[0])->setValueOnly(ijk, v);
//         } else if (this->isCached<NodeT1>(dirty)) {
//             // printf("node1");
//             ((NodeT1*)mNode[1])->setValueAndCache(ijk, *this, v);
//         } else if (this->isCached<NodeT2>(dirty)) {
//             // printf("lea2");
//             ((NodeT2*)mNode[2])->setValueAndCache(ijk, *this, v);
//         } else {
//             // printf("root");
//             mRoot->setValueAndCache(ijk, *this, v);
//         } 
//     }
// **************************************************************************************************
// **************************************************************************************************
