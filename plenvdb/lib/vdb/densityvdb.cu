...
__device__ float* digLeafDataFromAcc(const FloatAccT& acc, const CoordT& coord, FloatLeafT* leaf_src){
...
__device__ void accumulate(const FloatAccT& acc, const CoordT& coord, float val, FloatLeafT* leaf_src){
...
__global__ void density_copyFromDense_kernel(FloatGridT* grid, float* arr, const int rx, const int ry, const int rz, const int nleafCount){
...
void density_copyFromDense(FloatGridT* grid, float* arr, const int rx, const int ry, const int rz, const int nleafCount){
    density_copyFromDense_kernel<<<GET_BLOCK_NUM(nleafCount, BLOCKDIM), BLOCKDIM>>>(grid, arr, rx, ry, rz, nleafCount);
    cudaDeviceSynchronize();
}


__global__ void setValuesOn_bymask_cuda_kernel(FloatGridT* grid, bool* mask, const float val, const int rx, const int ry, const int rz, const int nleafCount){
    const int n = blockDim.x * blockIdx.x + threadIdx.x;
    if (n < nleafCount){
        const int nleaf = n >> 9;
        const int nvox = n & 511;
...
void setValuesOn_bymask_cuda(FloatGridT* grid, bool* mask, const float val, const int rx, const int ry, const int rz, const int nleafCount){
...
__global__ void density_forward_kernel(
    float* gpuRes, float* xs, float* ys, float* zs, FloatGridT* gpuGrid, const int N){
        const int n = blockDim.x * blockIdx.x + threadIdx.x;
        if (n < N){
...
void density_forward(float* gpuRes, float* xs, float* ys, float* zs, FloatGridT* gpuGrid, const int N){
...
__global__ void density_backward_kernel(float* grads, float* xs, float* ys, float* zs, FloatGridT* grid, const int N){
    const int n = blockDim.x * blockIdx.x + threadIdx.x;
    if (n<N){
...
void density_backward(float* grads, float* xs, float* ys, float* zs, FloatGridT* deviceGrid, const int N){
...
__global__ void density_updateData_kernel(
    FloatGridT* grid_data, FloatGridT* grid_grad, FloatGridT* grid_expavg, FloatGridT* grid_expavgsq,
    const float stepsz, const float eps, const float beta0, const float beta1, const int nleafCount){
        const int n = blockDim.x * blockIdx.x + threadIdx.x;
        if (n < nleafCount){
            const int nleaf = n >> 9;
            const int nvox = n & 511;
...
void density_updateData(
    FloatGridT* data, FloatGridT* grad, FloatGridT* exp_avg, FloatGridT* exp_avg_sq,
    const float stepsz, const float eps, const float beta0, const float beta1, const int nleafCount){
...
__global__ void density_updateDataWithPerlr_kernel(
    ...
    const float stepsz, const float eps, const float beta0, const float beta1, const int nleafCount, nanovdb::NanoGrid<float>* grid_per_lr){
        const int n = blockDim.x * blockIdx.x + threadIdx.x;
        if (n < nleafCount){
            const int nleaf = n >> 9;
            const int nvox = n & 511;
...
void density_updateDataWithPerlr(
    FloatGridT* data, FloatGridT* grad, FloatGridT* exp_avg, FloatGridT* exp_avg_sq,
    const float stepsz, const float eps, const float beta0, const float beta1, const int nleafCount, FloatGridT* per_lr){
...
__global__ void density_zero_grad_kernel(FloatGridT* gradGrid, const int nleafCount){
    const int n = blockDim.x * blockIdx.x + threadIdx.x;
    if (n < nleafCount){
        const int nleaf = n >> 9;
        const int nvox = n & 511;
...
void density_zero_grad(FloatGridT* gradGrid, const int nleafCount){
...
__global__ void density_forward_single_kernel(float* gpuRes, int* gpuPosi, int* gpuPosj, int* gpuPosk, FloatGridT* gpuGrid, const int N){
        const int n = blockDim.x * blockIdx.x + threadIdx.x;
        if (n < N){
            auto acc = gpuGrid->getAccessor();
...
void density_forward_single(float* gpuRes, int* gpuPosi, int* gpuPosj, int* gpuPosk, FloatGridT* gpuGrid, const int N){
...
void density_updateDataWithPerlr(
    FloatGridT* data, FloatGridT* grad, FloatGridT* exp_avg, FloatGridT* exp_avg_sq,
    const float stepsz, const float eps, const float beta0, const float beta1, const int rx, const int ry, const int rz, FloatGridT* per_lr){
