...
__global__ void prepare_accs_kernel(FloatGridT* densityGrid, Vec3fGridT** colorGrids, FloatGridT* maskGrid, 
    FloatAccT* densityAccs, Vec3fAccT* colorAccs, FloatAccT* maskAccs, int HW, int nVec){
...
void prepare_accs(FloatGridT* densityGrid, Vec3fGridT** colorGrids, FloatGridT* maskGrid, 
    FloatAccT* densityAccs, Vec3fAccT* colorAccs, FloatAccT* maskAccs, int HW, int nVec){
    prepare_accs_kernel<<<GET_BLOCK_NUM(HW, BLOCKDIM), BLOCKDIM>>>(densityGrid, colorGrids, maskGrid, densityAccs, colorAccs, maskAccs, HW, nVec);
    cudaDeviceSynchronize();
}


__global__ void prepare_accs_kernel(FloatGridT* densityGrid, Vec3fGridT** colorGrids, 
    FloatAccT* densityAccs, Vec3fAccT* colorAccs, int HW, int nVec){
    const int n = blockDim.x * blockIdx.x + threadIdx.x;
    if (n < HW){
        densityAccs[n] = densityGrid->getAccessor();
        for (int d=0; d<nVec; d++)
...
void prepare_accs(FloatGridT* densityGrid, Vec3fGridT** colorGrids, 
    FloatAccT* densityAccs, Vec3fAccT* colorAccs, int HW, int nVec){
    prepare_accs_kernel<<<GET_BLOCK_NUM(HW, BLOCKDIM), BLOCKDIM>>>(densityGrid, colorGrids, densityAccs, colorAccs, HW, nVec);
    cudaDeviceSynchronize();
}

__device__ float trigetDenValue(FloatAccT& acc, const Vec3fT& xyz, float* scales){
...
__device__ Vec3fT trigetColValue(Vec3fAccT& acc, const Vec3fT& xyz, float* scales){
    CoordT ijk = xyz.floor(); 
    Vec3fT uvw = xyz - ijk.asVec3s();
    Vec3fT val(0);
    val += acc.getValue(ijk) * scales[0]; ijk[2] += 1;
...
__device__ void get_tminmax(float* t_min, float* t_max, float* rays_d, float* rays_o, const float near, const float far)
{
