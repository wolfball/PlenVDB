...
from .grid import VDBGrid

from .masked_adam import MaskedAdam, VDBAdam
...
def create_optimizer_or_freeze_model(model, cfg_train, global_step):
    ...
    rgbnetopt, vdbopt = None, None
    if len(param_group) > 0:
        rgbnetopt = MaskedAdam(param_group)
    if len(vdb_param_group) > 0:
        vdbopt = VDBAdam(vdb_param_group)
    return rgbnetopt, vdbopt


''' Checkpoint utils
'''
def load_checkpoint(model, optimizer, vdbopt, ckpt_path, no_reload_optimizer):
    ckpt = torch.load(ckpt_path)
    start = ckpt['global_step']
    model.load_state_dict(ckpt['model_state_dict'])
    if vdbopt is not None:
        model.load_from(ckpt_path[:-9])
    if not no_reload_optimizer:
        if ckpt['optimizer_state_dict'] is not None:
            optimizer.load_state_dict(ckpt['optimizer_state_dict'])
        if vdbopt is not None:
            vdbopt.load_from(ckpt_path[:-9])
