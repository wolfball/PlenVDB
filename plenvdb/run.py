...
from imageio.core.util import BaseProgressIndicator
...
def load_existed_model(args, cfg, cfg_train, reload_ckpt_path):
    ...
    return model, optimizer, vdbopt, start
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    if reload_ckpt_path is None:
        ...
        model, optimizer, vdbopt = create_new_model(cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path)
        start = 0
        if cfg_model.maskout_near_cam_vox:
            model.maskout_near_cam_vox(poses[i_train,:3,3], near)
    else:
        print(f'scene_rep_reconstruction ({stage}): reload from {reload_ckpt_path}')
        model, optimizer, vdbopt, start = load_existed_model(args, cfg, cfg_train, reload_ckpt_path)
    ...
    if cfg_train.pervoxel_lr:
        def per_voxel_init():
            ...
            if vdbopt is not None:
                vdbopt.set_pervoxel_lr(cnt)
            else:
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step%args.i_print==0 or global_step==1:
            ...
            tqdm.write(f'scene_rep_reconstruction ({stage}): iter {global_step:6d} / '
                       ...
                       f'Eps: {eps_time_str}')
            time_record.append([global_step, eps_time, np.mean(psnr_lst), loss.item()])
            psnr_lst = []
    ...
    if global_step != -1:
        model.getTimer()
        if vdbopt is not None:
            model.save_to(last_ckpt_path[:-9])
            vdbopt.save_to(last_ckpt_path[:-9])
        
        model_state_dict = model.state_dict()
        torch.save({
            'time_recoder': time_record,
            'global_step': global_step,
            'model_kwargs': model.get_kwargs(),
            'model_state_dict': model_state_dict,
            'optimizer_state_dict': optimizer.state_dict() if optimizer is not None else None,
